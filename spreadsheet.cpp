...
bool Spreadsheet::readFile(const QString &fileName) {
    ...
    if (!file.open(QIODevice::ReadOnly)) {
        QMessageBox::warning(this, tr("Spreadsheet"), tr("Cannot read file").arg(file.fileName()).arg(file.errorString()));
        return false;
    }
    QDataStream in(&file);
    ...
    if (magic != MagicNumber) {
        QMessageBox::warning(this, tr("Spreadsheet"), tr("This file is not a Spread file"));
        return false;
    }
...
void Spreadsheet::paste() {
    ...
    int numCloumns = rows.first().count('\t') + 1;

    if (range.rowCount() * range.columnCount() != 1 && (range.rowCount() != numRows || range.columnCount() != numCloumns)) {
        QMessageBox::information(this, tr("Spreadsheet"), tr("This information cannot be paste"));
        return;
    }
